...
{ !ndata.length &&  <ScrollView style={{flex:1,}} 
...
{ item.applicant_id.cv.experience && item.applicant_id.cv.experience.map( ( val, key )=>{
    ...
    <Text style={{padding:5,paddingLeft:5,color:'#000',width:90,fontSize:12}}> {val.organization}</Text>
        ...
        <View style={{display:'flex',flexDirection:'row',justifyContent:'center',alignItems:'space-between',marginTop:10}}>
            
            <TouchableOpacity onPress={()=>this._updateStatus(item._id,"Dismiss Selection")} >
            <Text style={{fontSize:16,fontWeight:'bold',color:"yellow"}}>Dismiss Selection</Text>
            </TouchableOpacity>
